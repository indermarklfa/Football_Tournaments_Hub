...
import { createOrganiser } from '../../lib/api';
...
export default function NewOrganiser() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createOrganiser({ name, description: description || null, location: location || null });
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4" data-testid="new-organiser-page">
      <h1 className="text-2xl font-bold text-white mb-6">New Organiser</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-slate-800 p-6 rounded-lg space-y-4">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={loading}
            ...
            data-testid="submit-btn">{loading ? 'Creating...' : 'Create Organiser'}</button>
